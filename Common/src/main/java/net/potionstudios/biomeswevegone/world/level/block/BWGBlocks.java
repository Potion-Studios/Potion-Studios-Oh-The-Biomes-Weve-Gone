...
public class BWGBlocks {
   ...
   /** Bellflowers */
    ...
    public static final Supplier<VineBlock> SKYRIS_VINE = registerBlockItem("skyris_vine", () -> new VineBlock(BlockBehaviour.Properties.copy(Blocks.VINE)));
    ...
    private static FlowerBlockFeature registerFlower(String key, VoxelShape shape) {
        Supplier<? extends Block> flower = registerBlockItem(key, () -> new BWGFlowerBlock(BlockBehaviour.Properties.copy(Blocks.WHITE_TULIP).noOcclusion(), BlockTags.DIRT, shape));
        return new FlowerBlockFeature(key, flower);
    }

    private static FlowerBlockFeature registerFlower(String key, VoxelShape shape, Supplier<? extends Block> growAble) {
        Supplier<? extends Block> flower = registerBlockItem(key, () -> new BWGBonemealableFlowerBlock(BlockBehaviour.Properties.copy(Blocks.WHITE_TULIP).noOcclusion(), BlockTags.DIRT, shape, growAble));
        return new FlowerBlockFeature(key, flower);
    }
    ...
    private static Supplier<TallFlowerBlock> registerTallFlower(String key) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlock(BlockBehaviour.Properties.copy(Blocks.SUNFLOWER).noOcclusion()));
    }

    private static Supplier<TallFlowerBlock> registerTallFlower(String key, Supplier<AbstractTreeGrower> treeGrower) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlockTreeGrower(BlockBehaviour.Properties.copy(Blocks.SUNFLOWER).noOcclusion(), treeGrower));
    }

    private static Supplier<TallFlowerBlock> registerTallGlowingFlower(String key) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlock(BlockBehaviour.Properties.copy(Blocks.SUNFLOWER).lightLevel(state -> 10).noOcclusion()));
    }
