...
public class BWGBlocks {
   ...
   /** Bellflowers */
    ...
    public static final PottedBlock WINTER_CYCLAMEN = createPottedVariant("winter_cyclamen", () -> new BWGPlacementBushBlock(BlockBehaviour.Properties.of().sound(SoundType.GRASS).strength(0.0f).noCollission().noOcclusion().setId(key("winter_cyclamen")), Block.box(2.0D, 0.0D, 2.0D, 14.0D, 14.0D, 14.0D), BWGBlockTags.SNOWY_PLANT_PLACEABLE));
    ...
    private static FlowerBlockFeature registerFlower(String key, MapColor mapColor) {
        Supplier<? extends Block> flower = registerBlockItem(key, () -> new BWGFlowerBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.WHITE_TULIP).mapColor(mapColor).noOcclusion().setId(key(key))));
    ...
    private static FlowerBlockFeature registerFlower(String key, MapColor mapColor, VoxelShape shape) {
        Supplier<? extends Block> flower = registerBlockItem(key, () -> new BWGFlowerBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.WHITE_TULIP).mapColor(mapColor).noOcclusion().setId(key(key)), BlockTags.DIRT, shape));
        return new FlowerBlockFeature(key, flower);
    }
    ...
    private static FlowerBlockFeature registerFlower(String key, MapColor mapColor, VoxelShape shape, Supplier<? extends Block> growAble) {
        Supplier<? extends Block> flower = registerBlockItem(key, () -> new BWGBonemealableFlowerBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.WHITE_TULIP).mapColor(mapColor).noOcclusion().setId(key(key)), BlockTags.DIRT, shape, growAble));
        return new FlowerBlockFeature(key, flower);
    }
    ...
    private static Supplier<TallFlowerBlock> registerTallFlower(String key, MapColor mapColor) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.SUNFLOWER).mapColor(mapColor).noOcclusion().setId(key(key))));
    }

    private static Supplier<TallFlowerBlock> registerTallFlower(String key, MapColor mapColor, Supplier<TreeGrower> treeGrower) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlockTreeGrower(BlockBehaviour.Properties.ofFullCopy(Blocks.SUNFLOWER).mapColor(mapColor).noOcclusion().setId(key(key)), treeGrower));
    }

    private static Supplier<TallFlowerBlock> registerTallGlowingFlower(String key, MapColor mapColor) {
        return registerBlockItem(key, () -> new BWGTallFlowerBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.SUNFLOWER).mapColor(mapColor).lightLevel(state -> 10).noOcclusion().setId(key(key))));
    }
