...
enum BWGPlantProvider implements IBlockComponentProvider {
	...
	public void appendBody(ITooltip tooltip, IBlockAccessor accessor, IPluginConfig config) {
		...
		if (config.getBoolean(ResourceLocation.withDefaultNamespace("plant.crop_growable")))
			if (block instanceof CropBlock || block instanceof StemBlock)
				addCropGrowableTooltip(tooltip, accessor);

		if (config.getBoolean(ResourceLocation.withDefaultNamespace("plant.tree_growable")))
